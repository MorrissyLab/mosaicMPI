...
If no counts data is available and the dataset is normalized(eg., for non-count based assays), you can also specify `--is_normalized` to prevent mosaicMPI from performing a TPM
...
The default behaviour is to select overdispersed genes using an od-score > 1.0. You can also choose from a number of different methods for selecting overdispersed_genes, for example:
  ...
  - `mosaicmpi set-parameters --name example_run -m cnmf_topn -p 2000`: select top 2000 genes using cNMF's model and minimal mean threshold (Kotliar et al., 2019, eLife)
  - `mosaicmpi set-parameters --name example_run -m default_quantile -p 0.8`: Select top 20% of genes when ranked by od-score
  - `mosaicmpi set-parameters --name example_run -m genes_file -p path/to/genesfile.txt`: use a custom list of genes
...
This step will create annotated heatmaps of GEP usages from mosaicMPI outputs:
...
A [TOML](https://toml.io/en/) configuration file is the most flexible way to configure mosaicMPI. An example is found in `scripts/example_config.toml`. The minimal data to include in a TOML file is the paths to each of the datasets. Default values for other parameters will be used if not specified. 
...
To run integrative analyses on multiple datasets, you can run the following command using the same output directory you specified for `mosaicmpi integrate`.

```bash
mosaicmpi create-network -o output_directory -n name
```
