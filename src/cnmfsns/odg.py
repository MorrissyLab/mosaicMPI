...
def model_overdispersion(adata, odg_default_spline_degree=3, odg_default_dof=8, odg_cnmf_mean_threshold=0.5, selected_genes = []):
    ...
    df.loc[df["vscore_excluded"], "vscore"] = 0
    ...
    sns.histplot(df, x="log_mean", y="log_variance", bins=[100,100], ax=ax)
    ax.plot(df["log_mean"], df["gam_fittedvalues"], color="red")
    ax.set_title("Mean-Variance")
    ax.set_xlabel("log10(mean)")
    ax.set_ylabel("log10(variance)")
    ax.legend(handles=[Line2D([0], [0], color='red', label="model")])
    ax = axes[1]  # transformed
    sns.histplot(df, x="log_mean", y="odscore", bins=[100, 100], ax=ax)
    ax.hlines(1, xmin=df["log_mean"].min(), xmax=df["log_mean"].max(), color="red")
    ...
    ax.set_ylabel("log10(variance)")
    ax = axes[1]
    sns.histplot(df, x="log_mean", y="vscore", bins=[100, 100], ax=ax)
    ...
    sns.histplot(df, x="vscore", ax=ax)
